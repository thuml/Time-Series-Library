...
class PatchStateTemporalEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        patch_grid = patch_tokens.reshape(batch_size, self.in_channels, self.num_patches, -1)
        cls_hidden = encoded[:, 0, :]
        mean_hidden = patch_tokens.mean(dim=1)
        max_hidden = patch_tokens.amax(dim=1)
        last_hidden = patch_grid[:, :, -1, :].mean(dim=1)
        global_hidden = self.summary_proj(torch.cat([cls_hidden, mean_hidden, max_hidden, last_hidden], dim=-1))
        return patch_tokens, global_hidden
...
class Model(SGTONetV3Model):
    def __init__(self, configs):
        ...
        if self.d_model % n_heads != 0:
            n_heads = 1
        self.encoder = PatchStateTemporalEncoder(
