...
def main():
    parser = argparse.ArgumentParser(description="Run SGTONet and baseline future-state classification comparisons.")
    parser.add_argument(
        "--root_path",
        type=str,
    ...
    parser.add_argument("--output_root", type=str, default="/tmp/sgto_compare")
    parser.add_argument(
        "--models",
        type=str,
        default="DLinear,TimesNet,iTransformer,PatchTST,SGTONet,SGTONetV2,SGTONetV3,SGTONetV3Override,SGTONetV3Calibrated",
    ...
    for model_name in selected_models:
        ...
        des = f"futurecls_d{args.label_shift}_{model_name.lower()}"
        summary_path = None
        ...
        if summary_path is None:
            cmd = [sys.executable, "-u", "run.py"] + common + ["--model", actual_model_name, "--des", des] + variant["args"]
    ...
    rows = sorted(rows, key=lambda item: item.get("macro_f1", -1), reverse=True)
    out_path = os.path.join(output_root, "future_state_compare_summary.csv")
    headers = [
        "model",
        "label_shift",
        "accuracy",
    ...
    for idx, row in enumerate(rows, start=1):
        print(
            f"{idx}. {row['model']}: "
            f"macro_f1={row.get('macro_f1', 'NA')}, "
            f"balanced_accuracy={row.get('balanced_accuracy', 'NA')}, "
            f"class9_recall={row.get('class9_recall', 'NA')}"
        )
