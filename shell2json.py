...
def get_parser():
    parser = argparse.ArgumentParser(description='Tslib-json')
    ...
    parser.add_argument('--conv_kernel', type=int)
    # optimization
...
def sh2json(path):
    ...
    shell = [shell[i] for i in range(len(shell)) if shell[i] != '\n']
    shells = []
    model_name = shell[2].replace("model_name=", "")
    ...
    shells = [shell[shells[i] + 1:shells[i + 1] if i + 1 < len(shells) else len(shell)] for i in
              range(1, len(shells), 2)]
